...
class MultiOccurencesTaskTestCase(TestCase):
    ...
    def test_mot_modifications_modifies_related_tasks_number_a_day(self):
        ...
        # case de1 < de2 < ds1 < ds2 (cf models.py method modify_related_tasks)
        mot.start_date = date(2025, 1, 2)
        mot.end_date = date(2025, 1, 4)
        mot.save()
        related_tasks = DatedTask.objects.filter(related_mot=mot)
        self.assertEqual(related_tasks.filter(date=date(2025, 1, 2)).count(), 2)
        self.assertEqual(related_tasks.filter(date=date(2025, 1, 4)).count(), 2)
        self.assertEqual(related_tasks.filter(date=date(2025, 1, 3)).count(), 2)
        self.assertEqual(len(related_tasks), 6)
        # case de1 < ds1 < de2 < ds2
        mot.start_date = date(2025, 1, 5)
        mot.end_date = date(2025, 1, 9)
        mot.save()
        related_tasks = DatedTask.objects.filter(related_mot=mot)
        self.assertEqual(len(related_tasks), 10)
        self.assertEqual(related_tasks.filter(date=date(2025, 1, 5)).count(), 2)
        self.assertEqual(related_tasks.filter(date=date(2025, 1, 6)).count(), 2)
        self.assertEqual(related_tasks.filter(date=date(2025, 1, 7)).count(), 2)
        self.assertEqual(related_tasks.filter(date=date(2025, 1, 8)).count(), 2)
        self.assertEqual(related_tasks.filter(date=date(2025, 1, 9)).count(), 2)
        # case de2 < de1 < ds2 < ds1
        mot.start_date = date(2025, 1, 4)
        mot.end_date = date(2025, 1, 8)
        mot.save()
        related_tasks = DatedTask.objects.filter(related_mot=mot)
        self.assertEqual(len(related_tasks), 10)
        self.assertEqual(related_tasks.filter(date=date(2025, 1, 4)).count(), 2)
        ...
        self.assertEqual(related_tasks.filter(date=date(2025, 1, 7)).count(), 2)
        self.assertEqual(related_tasks.filter(date=date(2025, 1, 8)).count(), 2)
        # case de1 < de2 < ds2 < ds1
        mot.start_date = date(2025, 1, 5)
        mot.end_date = date(2025, 1, 7)
        mot.save()
        related_tasks = DatedTask.objects.filter(related_mot=mot)
        self.assertEqual(len(related_tasks), 6)
        self.assertEqual(related_tasks.filter(date=date(2025, 1, 5)).count(), 2)
        ...
        self.assertEqual(related_tasks.filter(date=date(2025, 1, 7)).count(), 2)
        self.assertEqual(related_tasks.filter(date=date(2025, 1, 8)).count(), 2)
        # case de2 < ds2 < de1 < ds1
        mot.start_date = date(2025, 1, 1)
        mot.end_date = date(2025, 1, 3)
        mot.save()
        related_tasks = DatedTask.objects.filter(related_mot=mot)
        self.assertEqual(len(related_tasks), 6)
        self.assertEqual(related_tasks.filter(date=date(2025, 1, 1)).count(), 2)
        self.assertEqual(related_tasks.filter(date=date(2025, 1, 2)).count(), 2)
        self.assertEqual(related_tasks.filter(date=date(2025, 1, 3)).count(), 2)
        # Changing reccurences should delete and create new tasks.
        mot.number_a_day = 3
    ...
    def test_related_tasks(self):
        """
        Make sure that related tasks are correctly counted.
        """
        start = date(2025, 1, 1)
        end = date(2025, 1, 17)
        # case of week tasks
        ...
        self.assertEqual(mot.related_tasks_count, 6)
        # case of dated tasks
        mot2 = MultiOccurencesTask.objects.create(
            name='mot',
            task_name='task',
        ...
        self.assertEqual(mot2.related_tasks_count, 3)
